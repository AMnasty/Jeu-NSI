...
perso = perso_droit
anvil = pygame.image.load("anvil.png")
...
bullet2scale = pygame.transform.scale(bullet2,(100,46))
...
def val_0():
    global pos_perso, lvl, drop, bullet_xy, a, b, vel, jump 
    pos_perso = (100,700)
    lvl = 0
    drop = (500,-100)
    bullet_xy = (1500,700)
...
def val_1():
    global pos_perso, lvl, drop, bullet_xy, a, b, vel, limite, drops, jump, bullet_xy,bullets,bullets2
    pos_perso = (100,700)
    drops = []
    bullets,bullets2 = [],[]
    lvl = 1
    ...
    for i in range(7):
        drops.append((drop[0]-50*i,drop[1]-100*i))
    for i in range(2):
                bullets.append((random.randint(1500,2000),(random.randint(700-23,700))))
                bullets2.append((random.randint(-1100,-500),(random.randint(700-23,700))))
    jump = False
...
def dessiner_niv1():
    ...
    for drop in drops:
        fenetre.blit(anvil,drop)
    fenetre.blit(bullet2,bullet_xy)
    dessiner_star(a)
    for bul in bullets:
        fenetre.blit(bullet,bul)
    for bul in bullets2:
        fenetre.blit(bullet2scale,bul)
    if a == False:
            fenetre.blit(star,(100,500))
    pygame.display.flip()

def dessiner_star(bool):
    if bool == True:
        fenetre.blit(star,(1300,500))
    else:
        pass
...
while True:
    ...
    while lvl == 1:
        ...
        if position(0,350) == True:
            bullet_xy = (bullet_xy[0]+7,600)
            for i in range(len(drops)):
                drops[i] = (drops[i][0],drops[i][1]+20)
            for i in range(len(bullets)):
                bullets[i] = (bullets[i][0]-5,bullets[i][1])
            for i in range(len(bullets2)):
                bullets2[i] = (bullets2[i][0]+5,bullets2[i][1])
        ...
        if int((temps1 - time.time())%2) == 0:
            bullet_sprite = 0
        bullet_rect2 = pygame.Rect(bullet_xy[0],bullet_xy[1]+20,159,187)
        if hitbox_perso.colliderect(bullet_rect2) == True:
                niv = lvl
                lvl = -2
                temps = time.time()
        for i,pos in enumerate(bullets):
            bullets_rect = pygame.Rect(pos[0],pos[1],100,23)
            if hitbox_perso.colliderect(bullets_rect) == True :
                niv = lvl
                lvl = -2
                temps = time.time()
        for i,pos in enumerate(bullets2):
            bullets2_rect = pygame.Rect(pos[0],pos[1],100,23)
            if hitbox_perso.colliderect(bullets2_rect) == True :
                niv = lvl
                lvl = -2
                temps = time.time()
